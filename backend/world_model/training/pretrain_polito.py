"""
pretrain_polito.py pre-trains the volts/amps stems + GRU trunk on the 1,976 real
Polito spot welds via masked-timestep reconstruction + fault-bit prediction, and
saves the transfer artifact that warm-starts the world model (STEPS.md Step 6, D5).

CLI (from backend/):
  python -m world_model.training.pretrain_polito --tiny
  python -m world_model.training.pretrain_polito --epochs 30 --device mps
...
def main():
    ...
    torch.save({
        "transfer_state_dict": model.transfer_state_dict(),
        "channels": PRETRAIN_CHANNELS,
        "stem_dim": STEM_DIM,
        "hidden_dim": HIDDEN_DIM,
        "config": config,
        "test_metrics": {k: v for k, v in m.items() if k != "fault_confusion"},
    }, ckpt)
